function aboutMeAnimate(){
	...
	.add(TweenMax.to('section.information', 1, {opacity:1, delay:1}))
	...
	if (window.innerWidth<1300) {
		...
		var tween2 = new TimelineMax()
		.add(TweenMax.to('section.skill', 1, {opacity:1, delay:1}))
		.add(TweenMax.to('.head', 1, {opacity:1, delay:-1}))
		...
		var scene2 = new ScrollMagic.Scene({
			...
			// duration: 100,
			offset: 0, //指標位移
			triggerHook: 1, // 觸發位置 0-1,onLeave,onCenter,onEnter
			reverse: false, //動畫重複 default:true
		})
		.setTween(tween2)
		// .addIndicators({ name: "2 (duration: 0)" }) // 指標顯示
